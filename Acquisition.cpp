...
double GetEScore(const cv::Vec<uchar, 3>& ch1, const cv::Vec<uchar, 3>& ch2) {
	...
	for (int j = 0; j < 3; ++j) {
		...
		if (div > 0) {
			//const double p = (std::min(a, b) / div) + 1; // in 1..2
			//e[j] = p * lookup_log2_64_log2_1_v_5(p) - 1;
...
np.array([1,-1]) @ A @ np.array([1,-1]).T
3976

npch1n = npch1 / (A[0,0]**0.5)
npch2n = npch2 / (A[1,1]**0.5)

ch1ch2n = np.vstack((npch1n, npch2n))
B = ch1ch2n @ ch1ch2n.T

np.array([1,-1]) @ B @ np.array([1,-1]).T
